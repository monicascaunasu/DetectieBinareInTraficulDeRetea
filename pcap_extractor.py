...
def get_extensions(header):
    headers = [{'HEADER': 'd0cf', 'EXTENSION': 'doc'}, {'HEADER': '4d5a'
               , 'EXTENSION': 'exe'}, {'HEADER': '504b0304',
               'EXTENSION': 'zip'}, {'HEADER': '504b0506',
               'EXTENSION': 'zip'}]
    for i in headers:
        ...
        if not sessions:
         ...
         # checks all sessions for matches
            ...
            # matches if both IPs and both ports are in the packets
                ...
                if i['IP'].src in session.values() and i['IP'].dst \
                    in session.values() and i['TCP'].sport \
                    in session.values() and i['TCP'].dport \
                    in session.values():
                    session['PACKETS'].append(i)
                    session['CONVO_SIZE'] = session['CONVO_SIZE'] \
                        + i['IP'].len
                    found = True
    ...
    for i in sessions:
         ...
         # finds the length of one side of the session
            ...
            if j['IP'].src == i['IP1']:
               ...
               # saves the hex for pushing into a file
                    ...
                    ip1_session += unhexlify(j['TCP'
                            ].load).encode('hex_codec')
                except:
                    continue
            elif j['IP'].src == i['IP2']:
      ...
      # should try testing this with text files, right now ive only done it with binary.
        ...
        if ip1_len > ip2_len:
            ...
            file.close()
            print " Your file was saved as 'test%s.%s'.\n If it has an extension of unkown, add the file header and document type to the get_extensions function." \
                % (count, extension)
